...
This information allows us to create a base map, shown below in @context-maps.
...
Not all entries have a site address, however, most entries without a site address are unimproved properties, as shown in @missing-site-addr-summary.
...
property_data <- property_data %>%
  mutate(own_investment = str_detect(str_to_upper(Owner), "\\W(INC|LLC|LTD|CORP|LP|COMMUNIT|COMPANY|LLP|REM|INT|AUTHORITY|BOARD|UNIVERSITY|INVESTMENT|APARTMENTS)"),
         own_estate = str_detect(str_to_upper(Owner), "\\W(ESTATE|TRUST)") & !str_detect(str_to_upper(Owner), "REAL ESTATE"))
...
We can see from @owner-investment-summary that most properties owned by LLCs/corporations are not owner-occupied, but that approximately half of those owned by estates/trusts are owner-occupied, and about 3/5 of the properties owned by individuals are owner-occupied.
...
rental_info <- owner_info %>%
  ...
  unique() %>%
  mutate(n_own_units_adj = n_own_units - as.numeric(own_occupied),
...
@properties-per-owner shows the distribution of property ownership for non-owner-occupied properties in Lancaster county. There are <200 entities who own 4-5 properties, and the number of entities at each level of property ownership decreases sharply as the number of properties increases.
...
xbreaks <- unique(round(10^seq(min(log10(owner_info$n_own_properties)), 
                               max(log10(owner_info$n_own_properties)), 
                               length.out = 100)))
x_minor_breaks <- c(0, 1, 2, 3, 5, 10, 20, 30, 50, 100, 200, 300, 500)

y_breaks <- 10^(-1:6)
y_minor_breaks <- c(c(3, 5, 7) %*% t(c(1, 10, 100, 1000, 10000, 100000)))

rental_info %>%
ggplot(aes(x = n_own_properties)) + 
  geom_histogram(breaks = xbreaks) + 
  # scale_y_log10(breaks = y_breaks, labels = y_breaks, minor_breaks = y_minor_breaks) +
  ylab("# owners") + 
  # scale_x_log10(minor_breaks = x_minor_breaks) + 
...
```
```{r units-per-owner}
#| fig-cap: Distribution of unit ownership portfolio size, for those who own at least one property that they do not reside in.
xbreaks <- unique(round(10^seq(0, 
                               max(log10(rental_info$n_own_units_adj), na.rm = T), 
                               length.out = 100)))
x_minor_breaks <- c(0, 1, 2, 3, 5, 10, 20, 30, 50, 100, 200, 300, 500, 1000)
...
ggplot() + 
  ...
  geom_text(data = tmp2, aes(x = 1.5, y = c(70000, 30000), color = big, label = big)) + 
