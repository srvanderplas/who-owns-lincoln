...
property_data <- property_data %>%
  mutate(own_investment = str_detect(str_to_upper(Owner), "\\W(INC|LLC|TRUST|ESTATE|CORPORATION|COMPANY|LLP|REM|INT|AUTHORITY)"))

own_investment_summary <- property_data %>%
  group_by(PropertyClass, own_investment, own_occupied) %>%
...
ggplot(own_investment_summary, aes(x = own_investment, color = PropertyClass, fill = PropertyClass, y = count, alpha = own_occupied)) + 
  # facet_wrap(~own_occupied) + 
  geom_col() + 
  scale_alpha_discrete("Occupant", c(.5, 1)) + 
  ggtitle("Investment Indicators and Property Address Correspondance") + 
  xlab("Investment Ownership") + 
  scale_fill_discrete("Property Type") + 
  scale_color_discrete("Property Type")
```

While it is certainly more likely that a property owned by an investment entity will be occupied by someone other than the owner, it is not a guarantee; there are a reasonable number of e.g. life estate/estate planning solutions that result in the owner occupying the premises.
