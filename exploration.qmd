...
A map of Nebraska roads was obtained from Nebraska's [open data site](https://www.nebraskamap.gov/datasets/dc69abca3b1a43749dea2141a3544bc9_0/explore) to provide contextual cues for generated maps. 
...
if (!file.exists("data/lancaster-co-rivers.geojson" | 
                 !file.exists("data/lancaseter-co-roads.geojson") | 
                 !file.exists("data/lancaseter-co-lakes.geojson"))) {
  ...
  write_sf(county_rivers, "data/lancaster-co-rivers.geojson")
  write_sf(county_roads, "data/lancaster-co-roads.geojson")
  write_sf(county_lakes, "data/lancaster-co-lakes.geojson")
} else {
  county_rivers <- st_read("data/lancaster-co-rivers.geojson") %>%
    st_transform(common_proj4string)
  county_roads <- st_read("data/lancaster-co-roads.geojson") %>%
    st_transform(common_proj4string)
  county_lakes <- st_read("data/lancaster-co-lakes.geojson") %>%
    st_transform(common_proj4string)
}
...
tmp %>%
  ...
  geom_sf(data = ne_counties, color = "black", fill = "transparent") + 
  geom_sf(data = county_lakes, fill = "blue") + 
  geom_sf(data = county_rivers, color = "blue") + 
  geom_sf(data = county_roads, color = "grey") + 
