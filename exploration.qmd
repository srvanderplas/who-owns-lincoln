...
read_zip_sf <- function(file){
  ...
  read_sf(tmp, layer = "Ownership Parcels")
}
...
missing_parcels <- lc_data %>%
  ...
  anti_join(shapefiles, by = c("Parcel_ID"= "PARCELID"))
```

It may make sense to systematically download these, searching via subblock or some other dimension. Doing it manually seems to have missed a significant number of parcels. 
