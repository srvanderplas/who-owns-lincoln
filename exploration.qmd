...
if (!file.exists("data/lancaster-co-features.geojson")) {
  ...
  county_roads <- roads %>%
    ...
    st_transform(common_proj4string) 
  county_roads <- st_difference(county_roads, lancaster$geometry)
...
# This is annoying - to use aesthetics to show the data we care about, we 
# need to actually *not* use aesthetics here. Which makes aggregating all of the
# data above and specifying the aesthetics in the data frame a waste of time...
# but changing it back would also be a waste of time. So onward.
...
```{r}
#| fig-cap: Map of property class type, for properties with corresponding spatial regions in the database. There are `r nrow(residential_properties)` in the overall dataset; `r nrow(missing_parcels)` of these properties do not have corresponding spatial information, and of those missing spatial information, `r filter(missing_parcels, PropertyClass == "Residential") %>% nrow()` are residential. 
